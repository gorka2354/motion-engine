...
- **`npm test`** — юнит-тесты (vitest, project `node`): геометрия камеры/орбит (инварианты граблей #6/#8 на числах Bybit), краевые anim-хелперы, **детерминизм-guard** (grep footgun #1 по `src`), scene-graph (`@react-three/test-renderer`). Чистая логика, без рендера.
- **`npm run test:layout`** — layout-тесты в **реальном Chromium** (vitest browser mode, `*.layout.test.tsx`): `getBoundingClientRect` ловит «элемент вылез за контейнер» (footgun #11) — jsdom и пиксельный `check-render` этот класс НЕ видят.
...
## Архитектура (`src/`)
- `Root.tsx` — реестр композиций: `TixuPromoV2` (44с, главный, **schema+defaultProps**), `TixuPromo` (V1 30с, светлая тема), 6 сцен V1, **`LibSandbox`** (стенд примитивов inc-2), **`FxSandbox`** (стенд переходов inc-6), **`InteractionSandbox`** (курсор/тап/spotlight), **`DataSandbox`** (Counter/SplitCompare/BarStat). `index.ts` — `registerRoot`.
...
- **`lib/` — премиум-примитивы (вход: `src/lib/index.ts`):**
  ...
  - **Interaction (UI-демо, стенд `InteractionSandbox`):** `<Cursor from to moveStart moveDur | stops>` — едущий курсор (eased-bezier, motion-trail, мульти-хоп через `stops`, прибывает за ~8f до тапа); `<TapPulse at x y>` — ripple+точка из одного progress (грабля #5); `<Spotlight cx cy shape="circle|rect">` — «гасит всё кроме» (радиал / скруглённый rect-вырез, цвет из `theme.dark` scrim); **`<TapTarget at x y from>`** — оркестратор: из ОДНОГО `at` разворачивает прибытие курсора + `TapPulse` (форсит синк). Press-scale цели — хелпер `tapScale(frame,fps,at)`. **⚠️ На accent-таргете курсор/пульс задавать контрастным цветом (accent-на-accent невидим).**
  - **Категории (Data-Stats/Comparison, стенд `DataSandbox`):** `<Counter to from prefix suffix decimals>` — BAN (roll-up, expo-out, tabular-nums, Intl-формат); `<SplitCompare left right at>` — before/after wipe (разделитель+хэндл, rest 50/50; контент разных сторон держать от шва — доку см. в файле); `<BarStat rows max>` — горизонтальные бары со stagger-ростом + счётчик значений в лок-степе.
  - Реэкспорт anim: `EASE/EASE_INOUT/EASE_OUT`, `kf`, `window01`, **`springWindow(f,fps,from,to,config)`** (spring-вход + eased-выход; raw `enter` овершутит — использовать для движения), **`SPRING`** (smooth/pop/bounce), **`DUR`** (tap/hop/reveal/settle — timing-ладдер @30fps), **`tapScale(frame,fps,at)`** (press-scale из одного spring), `stagger`, `stagger01`, `clamp01`.
...
- **⚠️ Грабли headless-рендера** (найдены эмпирически, детали в `src/lib/Grain.tsx`):
  ...
  11. **Flex-ребёнок без `minWidth:0` НЕ сжимается ниже ширины контента** (`min-width:auto` — дефолт flexbox) → длинный контент (напр. «Select...») распирает ряд, и сосед **вылезает за контейнер**. Пиксельный `check-render` это НЕ ловит (не знает «правильной» границы карточки), юнит — тоже (jsdom без layout). Детект: **browser-mode layout-тест** (`*.layout.test.tsx` · `npm run test:layout` — реальный Chromium, `getBoundingClientRect` проверяет, что дети внутри родителя). Правило: flex-дети → `minWidth:0`; клип-рамки → `overflow:hidden`. (Jumper `SwapWidget`: плашка To вылезала за карточку на ПУСТОМ состоянии — стилл заполненного проходил; класс «одно состояние ок, другое ломается».)
  12. **Наслаивание сцен на стыке (temporal overlap)** — континуальный компонент с magic-number таймингом (виджет уезжает по `kf`, текст появляется по другому окну) → окна перекрываются вручную, легко ошибиться (виджет ещё виден, а текст плюсов уже появился). **`check-render` это НЕ ловит** (оба слоя = движение; content/motion PASS). **Системное решение — sequencing: `@remotion/transitions` `<TransitionSeries>`** — каждая сцена в изолированном слоте, стыки = явные переходы (`fade`/`slide`), наслаивание невозможно by construction (образец: `src/jumper/JumperPromo.tsx` — Hook/Transfer/Benefits/CTA). Правило: многосценовый ролик = `TransitionSeries`, НЕ один компонент с ручными окнами. **⚠️ Выбор `presentation` критичен:** `fade()` КРОССФЕЙДИТ (оба слоя прозрачны на середине → для крупного центрального контента = визуальное наслаивание + «тупое гашение без движения»); для промо-сцен — **directional `slide({direction:"from-bottom"})`** (сцена уезжает вверх, новая въезжает снизу, контент РАЗДЕЛЁН по экрану, не пересекается). Тест: durations-math (`SCENES` сумма − transitions = duration; сцена > 2×XFADE) — `JumperPromo.test.tsx`. Флоу внутри сцены заканчивать за ~1с до её конца, чтобы финал бита («route found») держался перед переходом. **Проверять ВСЕ стыки на mid-transition кадрах глазами** (не только стиллы середины сцен) — этот класс стилл сцены не ловит.
...
- **Подпапка `motion-engine (наш движок)/`**: `00 — Статус`, `01 — Инструментарий`, `02 — 3D-пайплайн`, `03 — Тестирование`, **`04 — Категории роликов + структура`** (таксономия 10 типов + выбор в 3 вопроса + фреймворки), **`05 — Крафт`** (тайминг frame-математика · переходы · UI-приёмы cursor/tap/focus · эталоны Family/Robinhood/Stripe). Deep-research 6 агентов 2026-07-11.

## Скилл `motion-promo` (пайплайн промо-ролика)
`.claude/skills/motion-promo/SKILL.md` — от разведки URL до готового ролика: разведка(Playwright)→выбор категории→структура→бренд-пресет→сборка(sequencing/slide/spring)→тайминг(frame-математика)→проверка пирамидой→рендер. Вызывать при «сделай промо продукта». Детали крафта — вика 04/05.
