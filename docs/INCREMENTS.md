...
## inc-9 — Плавность склеек 🔄 (начат)
**Цель:** невидимые переходы между сценами/слоями (3D↔2D и любые cut'ы).
**Сделано (2026-07-08):** `lib/CutFlash` — световой блум-«выдох» поверх склейки (пик ровно на стыке, радиальный, peak 0.2–0.4) — классический монтажный приём; применён к 3D→2D стыку ShotikPromo (в связке с baked-texture match-cut из inc-8).
**Backlog приёмов (доучиться):** продолжение вектора движения через склейку (velocity continuation: следующий слой стартует с хвостом движения предыдущего); звуковой акцент на стыке (после inc-4 — свист/удар синхронно со вспышкой); J/L-cut логика (аудио/элемент следующей сцены начинается ДО визуальной склейки); motion-blur пик на кадре стыка; экспозиционный dip-to-glow вместо dip-to-black.
...
1. **still-vs-sequence** — 3–4 кадра рендерятся как stills И извлекаются из mp4 → пиксельная разница; расхождение = state-баг секвенциального рендера (r3f reparent и т.п.).
2. **motion-check** — соседние сэмплы (шаг ~15 кадров) должны отличаться: длинные отрезки с diff≈0 = «застыло/пропало».
3. **content-check** — дисперсия яркости центральной зоны каждого сэмпла: пустой центр = объект улетел из кадра.
4. **loop-check** (для лупов) — f0 vs fN-1 близки (шов).
...
- **Расширение пирамиды (2026-07-11, отдельные коммиты поверх):** приоритеты **1–4 реализованы** — **L3** `--gl=swangle`-пин (детерминизм 3D-пикселей) · **L4** golden-frame baseline (pixelmatch + SSIM-гейт 0.98, `--accept-baseline`, `test/golden/`) · **L5** 3D-asset QA (`npm run check-assets`: `gltf-validator` + metadata-фикстуры + Blender `_qa.py`) · **L2** scene-graph без GPU (`@react-three/test-renderer`: каркас `sceneGraph.test.tsx` + надстройка `BybitGif.test.tsx` — footgun #7 на живом `Tiles3D`/`Card3D`, мок `useCurrentFrame`, `Card3D`/`Tiles3D`/`tileOrbit` экспортированы Δ=0). Итого **32 vitest-теста**, 3 гейта `npm test`/`check-render`/`check-assets`; **L0–L5 автоматизированы**. Осталось: L3-доп (jank/title-safe/freezedetect) + L2 на реальном `useGltf`-loader'е (нужен DOM/delayRender). Полная карта — `docs/TESTING.md`.
