...
## L1b — Layout в реальном браузере (`npm run test:layout`, vitest browser mode)

jsdom **не считает layout** — «элемент вылез за контейнер» ему невидим (как и пиксельному
`check-render`, который не знает «правильной» границы карточки). Vitest browser mode
рендерит компонент в **настоящем Chromium** и мерит `getBoundingClientRect`.

```tsx
// src/**/*.layout.test.tsx — project "browser" (@vitest/browser-playwright)
const card = container.firstElementChild;
subCards(card).forEach((sc) => expect(overflowsX(sc, card)).toBe(false)); // дети внутри рамки
```

| Что ловит | Статус |
|---|---|
| **footgun #11** — flex-ребёнок без `minWidth:0` распирает ряд, сосед за контейнером (Jumper `SwapWidget` To за карточкой на пустом состоянии) | ✅ `SwapWidget.layout.test.tsx` |
| overflow клип-контейнеров, title-safe bounds на DOM-экранах (screens/, виджеты) | 🔲 расширять |

Проверено: тест **падает** с багом (без `minWidth:0`), проходит с фиксом — детектор доказанно срабатывает.
...
| **footgun #7 на ЖИВОМ компоненте** — реальный `Tiles3D` монтируется, node ушёл из GLB → `getObjectByName` undefined (обратная сторона reachability); + `Card3D`/`Tiles3D` version-compat smoke на пинах за мс | ✅ `BybitGif.test.tsx` |
| **data→transform интеграция** — смонтированная `position` == `tileOrbit()`, орбита сдвигается по Remotion-кадру (мок `useCurrentFrame`, т.к. сцены НЕ на `useFrame`) | ✅ `BybitGif.test.tsx` |
...
✅ **Реальный компонент поверх каркаса** (`src/bybit/BybitGif.test.tsx`, 3 теста): `Card3D`/`Tiles3D`
экспортированы (Δ=0, только `export`), Remotion `useCurrentFrame` мокается через `vi.hoisted` →
управляемый кадр, орбита двигается через `update()` (не `advanceFrames` — сцены Remotion-frame-driven).
GLB подаётся синтетическими нодами (реальный `GLTFLoader`/`useGltf` держит `delayRender`+DOM → вне
scope L2; загрузка ассета проверяется на L3 `--seq` / L5 `check-assets`).
...
## L5-audio — Аудио self-check (`npm run check-audio`, на озвученные компы)

Команда: **`npm run check-audio <CompId|file.mp4>`** — «ухо, которого у агента нет». Класс граблей «звук» невидим для стилла и `check-render` (см. footgun #13): дорожка молча пропала, клиппинг, слишком тихо/громко.

| Механизм | Что ловит | Статус |
|---|---|---|
| **`check-audio.mjs`** → встроенный `ffprobe` (наличие audio-stream) | `<Audio>` молча деградировал на Html5Audio / звук вообще не смуксился | ✅ есть |
| → `ffmpeg loudnorm=print_format=json` (integrated LUFS + true-peak), гейт [−20,−12] LUFS · TP ≤ −1 | пересолен/тихий микс, inter-sample клиппинг | ✅ есть |
| синтез-набор `npm run gen-sfx` печатает peak/RMS dBFS на клип | «клип = тишина / клиппит» ещё до композиции | ✅ есть |

⚠️ Встроенный ffmpeg Remotion — минимальная сборка: **нет `volumedetect`/`astats`**, но `loudnorm`/`volume` есть. `-f null` требует `-vn` (нет null-video-энкодера). Субъективное качество/синхрон на слух — только L6 (человек).
...
| #11 flex overflow за контейнер | **L1b** browser-mode `getBoundingClientRect` (`test:layout`) | ✅ |
| #12 наслаивание сцен на стыке | **архитектура** — `TransitionSeries` (сцены не перекрываются by construction) + durations-math тест | ✅ |
...
4. ✅ **`@react-three/test-renderer` smoke** — сделано (каркас `sceneGraph.test.tsx` + реальный `BybitGif.test.tsx`): ловит #7 без GPU.
...
geometry/anim/pixelMetrics · **L2 scene-graph** (`@react-three/test-renderer`: каркас + реальный BybitGif Tiles3D/Card3D, #7-guard) · L3
...
32 vitest-тестов · 3 гейта `npm test` / `check-render` / `check-assets`. Осталось (L3-доп, по приоритету):
