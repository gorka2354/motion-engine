...
(26 тестов) · L3 content/motion/loop/seq + **`--gl=swangle` пин** · L4 **golden-frame baseline**
(pixelmatch + SSIM-гейт 0.98, `--accept-baseline`, эталоны в `test/golden/`). Остальное — backlog
по приоритету выше (следующее: `_qa.py`+gltf-validate, `@react-three/test-renderer`).*
