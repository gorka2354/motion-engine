...
/**
 * Lumo 44s master, expressed as data (same engine as tixu — FloatingPhonePromo).
 * Fictional EdTech brand; copy is original (not the tixu wording). Timing/nav
 * reuse the proven camera choreography. All frames absolute @ 30fps.
 * `brand.logo` is unused (Lumo renders a text wordmark) — kept to satisfy the
 * shared schema.
 */
export const LUMO_DEFAULTS: PromoProps = {
  brand: {
    logo: "logo.svg", // unused — Lumo wordmark is text
