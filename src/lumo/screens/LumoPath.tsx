...
import { Icon } from "../LumoArt";

const L = theme.lumo;
const EASE = Easing.bezier(0.16, 1, 0.3, 1);

const W = 574; // phone screen inner width
const CX = W / 2;
const START_Y = 312;
const GAP = 178;

const NODES = [
  { label: "Your First Real Prompt", x: 0, active: true },
  { label: "Give the AI\nthe right context", x: -128 },
  { label: "Break big tasks\ninto steps", x: 122 },
  { label: "Ask for a\nbetter answer", x: -112 },
  { label: "Build a prompt\nyou can reuse", x: 104 },
];

export const LumoPath: React.FC = () => {
    ...
    <AbsoluteFill style={{ background: theme.color.surface, fontFamily: theme.font.stack }}>
            ...
            <div key={i}>
              <div
                style={{
                  ...
                  borderRadius: 999,
                  background: p.active
                    ? "linear-gradient(160deg, #FB8DA0 0%, #F1466A 100%)"
                    : "#F2EEF0",
                  boxShadow: p.active
                    ? `0 16px 30px -10px ${L.accentDeep}`
              ...
              >
                <Icon name="play" size={26} color={p.active ? "#fff" : "#B9AEB2"} />
              </div>
              {/* label */}
              <div
                style={{
      ...
      <div
        style={{
          ...
          paddingBottom: 18,
          zIndex: 20,
          boxShadow: "0 10px 20px -14px rgba(120,40,60,0.25)",
        }}
      >
        <div
          style={{
